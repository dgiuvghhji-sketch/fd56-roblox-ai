...
// دالة الاتصال المباشر والمستقرار بـ Gemini API
function callGeminiAPI(promptText, callback) {
    const apiKey = process.env.GEMINI_API_KEY || '';
    const postData = JSON.stringify({
        contents: [{ parts: [{ text: `أنت نظام الذكاء الاصطناعي الخاص بالمطور العبقري FD_56. رد بطريقة ذكية، فخمة، وودودة. الرد على: ${promptText}` }] }]
    });
        ...
        res.on('end', () => {
            try {
                ...
                if (parsed.candidates && parsed.candidates[0].content.parts[0].text) {
                    ...
                    callback(null, "أهلاً بك يا مبدع، أنا جاهز لأوامرك.");
                }
            } catch (e) {
                callback("خطأ في تحليل البيانات");
            }
        });
    });
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        ...
        .new-chat-btn { background: transparent; border: 1px solid var(--gold-primary); color: var(--gold-primary); padding: 10px; border-radius: 8px; cursor: pointer; font-weight: bold; width: 100%; margin-bottom: 20px; }
        ...
        .chat-input-area button { padding: 0 25px; background: linear-gradient(135deg, var(--gold-primary), var(--gold-secondary)); color: #000; font-weight: bold; border: none; border-radius: 10px; cursor: pointer; }
    </style>
</head>
<body>
...
app.post('/api/chat', (req, res) => {
    ...
    callGeminiAPI(message, (err, replyText) => {
        if (err) {
            res.status(500).json({ reply: "عذراً يا صاحبي، حدث خطأ في المعالجة." });
        } else {
...
app.listen(PORT, '0.0.0.0', () => {
    console.log(`🚀 FD_56 Server running on port ${PORT}`);
});
