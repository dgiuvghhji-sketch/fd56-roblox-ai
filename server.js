...
function callGeminiAPI(username, promptText, callback) {
    // الاعتماد المباشر على المتغير بالمسمى المطلوب
    ...
    // استخدام أحدث إصدارات النماذج مع مفتاح GEMINI_API_KEY
    const options = {
        ...
        res.on('end', () => {
            try {
                ...
                if (parsed.candidates && 
                    ...
                    callback(null, "خطأ من جوجل: " + (parsed.error.message || "تأكد من المفتاح"));
                } else {
