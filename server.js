...
// تهيئة مفتاح جيميناي (تأكد أنك ضايف GEMINI_API_KEY في Environment Variables في رندر)
const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY });

// قاعدة بيانات مؤقتة بالحسابات والمحادثات
let usersDB = [];
let chatsDB = {};

// 1. مسار الواجهة الأمامية الفخمة (تصميم أسود وذهبي + خلايا عصبية + نظام دخول وحفظ)
app.get('/', (req, res) => {
...
// 2. مسارات الـ API (تسجيل الحساب، تسجيل الدخول، وحفظ المحادثات)
...
app.post('/api/chat', async (req, res) => {
    ...
    try {
        ...
        const response = await ai.models.generateContent({
            ...
            contents: `أنت نظام الذكاء الاصطناعي الخاص بالمطور العبقري FD_56. رد بطريقة ذكية، فخمة، وودودة كبشري محترف وخبير برمجة. الرد: ${message}`,
        });

        const replyText = response.text || "أهلاً بك يا مبدع، أنا جاهز لأوامرك.";
