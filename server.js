...
function callGeminiAPI(username, promptText, callback) {
    ...
    const systemInstruction = `أنت نموذج ذكاء اصطناعي مطور ومساعد شخصي للمطور العبقري FD_56. أنت تتحدث الآن مع المستخدم المسمى "${username}". ناده باسمه واجعله محور اهتمامك وأجب على أسئلته بدقة واحترافية.`;
        ...
        res.on('end', () => {
            try {
                ...
                if (parsed.candidates && 
                    ...
                    callback(null, "عذراً، خطأ من جوجل: " + (parsed.error.message || "تأكد من المفتاح"));
                } else {
                    callback(null, "عذراً، لم يتمكن النموذج من الرد.");
                }
            } catch (e) {
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        ...
        #auth-container { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(5, 5, 5, 0.98); display: flex; justify-content: center; align-items: center; z-index: 200; }
        .auth-box { background: var(--panel-color); border: 2px solid var(--gold-primary); padding: 25px; border-radius: 16px; width: 90%; max-width: 350px; text-align: center; box-shadow: 0 0 30px rgba(212, 175, 55, 0.3); }
        ...
        /* القائمة الجانبية المخفية تفتح بالزر */
        sidebar { position: absolute; right: -280px; top: 0; width: 280px; height: 100%; background: rgba(15, 15, 15, 0.98); border-left: 1px solid rgba(212, 175, 55, 0.3); display: flex; flex-direction: column; padding: 20px; transition: 0.3s ease; z-index: 150; box-shadow: -5px 0 20px rgba(0,0,0,0.8); }
        ...
        .chat-main { flex-grow: 1; width: 100%; height: 100%; display: flex; flex-direction: column; background: rgba(5, 5, 5, 0.85); }
        .chat-header { padding: 12px 20px; background: rgba(15, 15, 15, 0.95); border-bottom: 1px solid rgba(212, 175, 55, 0.2); display: flex; justify-content: space-between; align-items: center; height: 60px; }
        .chat-header h1 { font-size: 16px; color: var(--gold-primary); }
        ...
        .message.ai { background: #181818; color: #fff; align-self: flex-end; border: 1px solid rgba(212, 175, 55, 0.3); }
        
        .chat-input-area { padding: 15px; background: rgba(15, 15, 15, 0.95); border-top: 1px solid rgba(212, 175, 55, 0.2); display: flex; gap: 10px; height: 75px; }
        .chat-input-area input { flex-grow: 1; padding: 12px; background: #121212; border: 1px solid #333; color: #fff; border-radius: 10px; outline: none; font-size: 14px; }
...
<body>
    ...
    <div id="app-container">
        <div class="chat-main">
            ...
            <div class="chat-messages" id="chat-messages">
                <div class="message ai">أهلاً بك يا أسطورة. أنا نموذج جوجل الذكي وجاهز لكل أسئلتك!</div>
            </div>
    ...
    <script>
        ...
        // زيادة وضوح الخلايا العصبية وسطوعها
        for (let i = 0; i < 55; i++) {
            particles.push({ x: Math.random() * canvas.width, y: Math.random() * canvas.height, vx: (Math.random() - 0.5) * 0.7, vy: (Math.random() - 0.5) * 0.7, radius: Math.random() * 2.5 + 1.2 });
        }
        ...
        function animateNeural() {
            ...
            particles.forEach((p, index) => {
                ...
                for (let j = index + 1; j < particles.length; j++) {
                    let p2 = particles[j];
                    if (Math.hypot(p.x - p2.x, p.y - p2.y) < 130) {
