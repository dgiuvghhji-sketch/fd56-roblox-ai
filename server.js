...
app.post('/api/chat', async (req, res) => {
    ...
    if (!message) return res.status(400).json({ reply: "بيانات ناقصة" });
    
    const user = username || "زائر";
    if (!chatsDB[user]) chatsDB[user] = [];
    chatsDB[user].push({ role: 'user', text: message });

    try {
        // تم تحديث اسم الموديل إلى gemini-2.5-flash
        const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" });
        const promptWithContext = `أنت مساعد ذكاء اصطناعي خاص بالنظام. تتحدث مع المستخدم "${user}". رد عليه مباشرة وبشكل مرتب.\nالمستخدم: ${message}`;
        
        const result = await model.generateContent(promptWithContext);
        const responseText = result.response.text();

        chatsDB[user].push({ role: 'ai', text: responseText });
        res.json({ reply: responseText });

    } catch (error) {
        res.status(500).json({ reply: "خطأ من جوجل: " + error.message });
    }
});
